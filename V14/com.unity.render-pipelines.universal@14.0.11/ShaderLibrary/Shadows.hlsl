...
#define VSM_GLOBAL
int _VsmMainEnabled;
TEXTURE2D(_VsmScreenMainShadow);
SAMPLER(sampler_VsmScreenMainShadow);
...
half MainLightShadow(float4 shadowCoord, float3 positionWS, half4 shadowMask, half4 occlusionProbeChannels)
{
	if (_VsmMainEnabled)
	{
		...
		float2 screenUV = positionCS.xy * 0.5f + 0.5f;
#if UNITY_UV_STARTS_AT_TOP
		screenUV.y = 1.0f - screenUV.y;
#endif
		float shadow = SAMPLE_TEXTURE2D(_VsmScreenMainShadow, sampler_VsmScreenMainShadow, screenUV.xy).x;
